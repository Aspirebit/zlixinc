...
-- Contact Messages
create policy if not exists "Anon can insert contact messages"
  on public.contact_messages
  for insert
  to anon
  with check (true);

create policy if not exists "Authenticated can read contact messages"
  on public.contact_messages
  for select
  to authenticated
  using (true);
...
-- Email Subscriptions
create policy if not exists "Anon can insert email subscriptions"
  on public.email_subscriptions
  for insert
  to anon
  with check (true);

create policy if not exists "Authenticated can read email subscriptions"
  on public.email_subscriptions
  for select
  to authenticated
  using (true);
...
create policy if not exists "Authenticated can update messages"
  on public.contact_messages
  for update
  to authenticated
  using (true)
...
create policy if not exists "Authenticated can update subscriptions"
  on public.email_subscriptions
  for update
  to authenticated
  using (true)
